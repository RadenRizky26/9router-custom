...
const hasSryRouterConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["SryRouter"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["SryRouter"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasSryRouter: hasSryRouterConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("SryRouter/") ? config.model.replace(/^SryRouter\//, "") : null,
...
// POST - Apply SryRouter as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_SryRouter";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing SryRouter provider entry and its models
    const existingProvider = config.provider["SryRouter"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["SryRouter"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `SryRouter/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `SryRouter/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("SryRouter/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove SryRouter provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["SryRouter"]?.models) {
      delete config.provider["SryRouter"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["SryRouter"].models).length === 0) {
        delete config.provider["SryRouter"];
        if (config.model?.startsWith("SryRouter/")) delete config.model;
      } else if (config.model === `SryRouter/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["SryRouter"].models);
        config.model = `SryRouter/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire SryRouter provider
      if (config.provider) delete config.provider["SryRouter"];
      if (config.model?.startsWith("SryRouter/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("SryRouter/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "SryRouter settings removed from OpenCode",
    });
  } catch (error) {
