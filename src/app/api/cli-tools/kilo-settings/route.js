...
const hasSryRouterConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["SryRouter"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("SryRouter");
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasSryRouter: hasSryRouterConfig(auth),
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "SryRouter", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "SryRouter settings removed from Kilo Code" });
  } catch (error) {
