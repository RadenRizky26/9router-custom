...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-SryRouter.env");
};
...
const hasSryRouterConfig = (config) => {
  ...
  if (providers["SryRouter"]) return true;
...
export async function GET() {
  ...
  const hasSryRouter = hasSryRouterConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_SryRouter_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile SryRouter",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["SryRouter"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_SryRouter_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "SryRouter configuration removed from jcode",
    });
  } catch (error) {
