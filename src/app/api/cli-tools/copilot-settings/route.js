...
const hasSryRouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "SryRouter");
};

const getSryRouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "SryRouter") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getSryRouterEntry(config);
...
// POST - Apply SryRouter config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing SryRouter entry or append
    const idx = config.findIndex((e) => e.name === "SryRouter");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove SryRouter entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "SryRouter");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "SryRouter removed from Copilot config",
    });
  } catch (error) {
