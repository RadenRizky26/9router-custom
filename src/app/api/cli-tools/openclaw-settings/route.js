...
// Check if settings has SryRouter config
const hasSryRouterConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["SryRouter"];
};

// Read per-agent models.json and return current model id (without "SryRouter/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["SryRouter"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasSryRouter: hasSryRouterConfig(settings),
...
// POST - Update SryRouter settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `SryRouter/${model}`;

    // Remove all old SryRouter/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("SryRouter/"))
    ...
    // Add fresh SryRouter models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`SryRouter/${m}`] = {};
    });

    // Remove old SryRouter model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("SryRouter/")) {
    ...
    // Update models.providers.SryRouter with all models
    settings.models.providers["SryRouter"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `SryRouter/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove SryRouter settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove SryRouter from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["SryRouter"];
    ...
    // Remove SryRouter models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("SryRouter/"));
    ...
    // Reset agents.defaults.model.primary if it uses SryRouter
    if (settings.agents?.defaults?.model?.primary?.startsWith("SryRouter/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "SryRouter settings removed successfully",
    });
  } catch (error) {
