...
const previousDefaultRegExp = /^# SryRouter-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
const previousSubagentRegExp = (type) =>
  new RegExp(
    `^# SryRouter-prev-subagent-${escapeRegExp(type)} = "([^"]*)"[ \\t]*\\r?\\n?`,
    "m",
  );
...
function buildModelSection({ slot, model, baseUrl, apiKey, contextWindow, name }) {
  const lines = [
    ...
    `description = ${tomlString("Routed via SryRouter gateway")}`,
...
function rememberPreviousDefault(toml) {
  ...
  return insertMarker(toml, `# SryRouter-prev-default = ${tomlString(current)}\n`);
}
...
function rememberPreviousSubagent(toml, type) {
  ...
  return insertMarker(
    toml,
    `# SryRouter-prev-subagent-${type} = ${tomlString(previous)}\n`,
  );
}
