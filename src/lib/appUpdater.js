  ...
  try {
    const mitmPidFile = path.join(
      process.platform === "win32"
        ? path.join(process.env.APPDATA || "", "SryRouter")
        : path.join(os.homedir(), ".SryRouter"),
      "mitm",
      ".mitm.pid"
    );
...
// Collect PIDs of all SryRouter-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from SryRouter install dir or wrapper cli.js
        const isAppProcess = lower.includes("SryRouter") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("SryRouter") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "SryRouter");
  }
  return path.join(os.homedir(), ".SryRouter");
}
...
// Resolve npx/SryRouter binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx SryRouter` — works regardless of global bin path changes after npm i -g
