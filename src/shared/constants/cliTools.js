...
export const CLI_TOOLS = {
  ...
  copilot: {
    ...
    color: "#1F6FEB",
    description: "GitHub Copilot in VS Code via SryRouter extension",
    configType: "guide",
    docsUrl: "https://marketplace.visualstudio.com/items?itemName=hotrungnhan.SryRouter-for-github-copilot",
    guideSteps: [
      {
        step: 1,
        title: "Install Extension",
        desc: "In VS Code, open Extensions (Ctrl+Shift+X or Cmd+Shift+X), search for 'SryRouter for Github Copilot' and click Install.",
      },
      {
        step: 2,
        title: "Configure Server",
        desc: "Press Cmd+Shift+P (or Ctrl+Shift+P), run 'SryRouter: Configure Server', then enter your Server URL and API Key:",
        value: "{{baseUrl}}",
        copyable: true,
      },
      {
        ...
        desc: "Open Copilot Chat, click the model picker at the bottom → 'Manage Models...' → check the SryRouter models to use.",
      },
    ],
  },
  opencode: {
  ...
  amp: {
    ...
    notes: [
      { type: "info", text: "Use SryRouter model aliases to keep Amp shorthand mappings stable across provider updates." },
    ...
    guideSteps: [
      ...
      { step: 5, title: "Add Shorthands", desc: "Map Amp shorthand names such as g25p or cs45 to SryRouter aliases in your local config." },
    ],
    codeBlock: {
      language: "bash",
...
# cs45 -> cc/claude-sonnet-4-5-20250929`,
  ...
  qwen: {
    ...
    color: "#10B981",
    description: "Alibaba Qwen Code CLI — supports OpenAI, Anthropic & Gemini providers via SryRouter",
    docsUrl: "https://qwenlm.github.io/qwen-code-docs/en/users/configuration/model-providers/",
    configType: "guide",
    defaultCommand: "qwen",
    notes: [
      { type: "info", text: "Qwen Code supports multiple provider types (openai, anthropic, gemini) via modelProviders in settings.json. SryRouter works as an OpenAI-compatible endpoint." },
      { type: "info", text: "Any model available in SryRouter can be used — not just Qwen models. Select from Qwen, Claude, Gemini, GPT, and more." },
      { type: "warning", text: "Config path: Linux/macOS ~/.qwen/settings.json • Windows %USERPROFILE%\\.qwen\\settings.json" },
      { type: "error", text: "Qwen OAuth free tier was discontinued on 2026-04-15. Use SryRouter with alicode/openrouter/anthropic/gemini providers instead." },
    ],
  ...
  "deepseek-tui": {
    ...
    notes: [
      { type: "info", text: "DeepSeek TUI uses ~/.deepseek/config.toml for configuration. SryRouter will update the provider to 'openai' mode with your base_url, api_key, and model." },
  ...
  jcode: {
    ...
    notes: [
      ...
      {
        type: "info",
        text: "Configure SryRouter as an OpenAI-compatible provider to route all jcode requests through SryRouter's optimization layer."
      },
      {
        type: "warning",
  ...
  "grok-build": {
    ...
    notes: [
      {
        type: "info",
        text: "Grok Build uses ~/.grok/config.toml. SryRouter writes a [model.SryRouter] custom model and sets it as the default.",
      },
      {
        type: "info",
        text: "After Apply, run grok (or /model SryRouter) to use the routed model. Switch back anytime with /model grok-build.",
      },
      {
        type: "warning",
...
devin --version`,
  ...
  opendesign: {
    ...
    configType: "guide",
    notes: [
      { type: "info", text: "OpenDesign ships as a plugin/skills pack installed into Claude Code, Cursor, OpenAI Codex, Gemini CLI, or OpenCode. It inherits the host agent's model config, so once your host points at SryRouter, /opendesign design sessions route through SryRouter automatically — no extra env vars needed." },
    ...
    guideSteps: [
      ...
      { step: 2, title: "No config needed", desc: "OpenDesign runs inside your host agent and uses its model config. If the host already routes through SryRouter, /opendesign traffic does too." },
